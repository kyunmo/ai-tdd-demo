...
├── generation-guide.md             # 공통 생성 가이드
├── document-guide.md               # 문서 가이드
...
graph TD
    ...
    S --> B["generation-guide.md<br>(생성 가이드 참조)"]
    B --> C{"3.클래스 유형 판별"}
...
| **핵심 내용** | 프로젝트명, 프레임워크/JDK 버전, 패키지 구조, 테스트 프레임워크, AI환경 |
| **커스터마이징** | **필수** - `[수정필요]` 표시 항목을 프로젝트에 맞게 설정 |
| **참조 대상** | 에이전트가 직접 참조 |

**수정 필요 항목**: 프로젝트명, 프레임워크 버전, JDK 버전, 빌드 도구 버전, 기본 패키지

### 2.2. `generation-guide.md` - 공통 생성 가이드
...
### 2.5. `references/examples/` - 참고 예제 (진행하면서 업데이트)
...
| `mapper-test-example.md` | `@Mapper` (UserMapper) | Mock 기반 + `@MybatisTest` DB연동, `@Sql`, CRUD 패턴 |
| `util-test-example.md` | Utility | `@ParameterizedTest`, `@CsvSource`, NH 마스킹 검증 |
...
| `generation-guide.md` | 100% 공통 | 수정 불필요 |
...
- `generation-guide.md`와 `templates/`는 전체 프로젝트에 영향을 미치므로 **공통 저장소에서 관리**
