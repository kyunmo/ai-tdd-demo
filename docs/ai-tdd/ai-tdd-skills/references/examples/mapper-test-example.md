...
> Mapper 인터페이스를 `@Mock`으로 처리하여 **CRUD 반환값 패턴과 예외 시나리오**를 검증하는 방식입니다.
> Mapper의 각 메서드가 다양한 상황(정상/null/빈 리스트/DB 오류)에서 어떻게 동작하는지 정의하며,
> 서비스 테스트에서 이 Mock 패턴을 그대로 재사용할 수 있습니다.
...
### 4.3. 4단계 레벨 분포 (Mock 기반)

| 레벨 | 테스트 수 | 테스트 메서드 |
|---|---|---|
| Level 1: Happy Case | 4개 | findById, findByEmail, findAll, insert |
| Level 2: Edge Case | 3개 | null 반환, 빈 리스트, 상태별 필터 |
| Level 3: Exception | 2개 | DataAccessException, update 0건 |
| Level 4: Mutation | 2개 | ArgumentCaptor insert 검증, deleteById 호출 검증 |

> 이 예제는 Mapper 계층의 CRUD 패턴을 모두 포함하기 위해 Level 4 비중이 목표(10%)보다 높습니다.
