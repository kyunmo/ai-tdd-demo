...
| 이메일 | 아이디 부분 마스킹 | `hong@nhbank.com` → `h***@nhbank.com` |
...
| 감사로그 대상 | 기록 항목 |
|---|---|
| 사용자 생성/수정/삭제 | 작업자, 작업 시간, 변경 내용 |
| 로그인/로그아웃 | 사용자 ID, IP, 시간, 성공/실패 |
| 권한 변경 | 변경 전/후 권한, 변경자 |
| 데이터 조회 (민감정보) | 조회자, 조회 대상, 시간 |
...
| API 키 | 하드코딩 금지 | `apiKey = "sk-xxxxx"` |
| DB 접속정보 | 하드코딩 금지 | `url = "jdbc:mysql://prod-db..."` |
...
String password = "Nh2026!@#$";      // 실제 비밀번호처럼 보임
String email = "hong@nhbank.com";     // 실제 이메일처럼 보임
```
