> **[중요] 임시 비활성화 알림**
>
> 현재 프로젝트는 JaCoCo 라이브러리 반입 대기 중입니다. 따라서 이 문서에 명시된 **테스트 커버리지 요구사항 및 검증 절차는 일시적으로 적용되지 않습니다.**
>
> JaCoCo가 도입된 후 이 알림은 제거될 것이며, 그때부터 이 문서의 모든 규칙이 다시 활성화됩니다. AI 에이전트는 현재 단계에서는 커버리지 검증을 시도하지 마십시오.
---
# 테스트 커버리지 요구사항
