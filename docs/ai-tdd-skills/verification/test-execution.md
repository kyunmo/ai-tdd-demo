> **[문서 역할 안내] 문제 해결 가이드: 테스트 실행 오류**
>
> 이 문서는 AI 에이전트가 자동화 검증 스크립트(`run-compile-test.sh`) 실행 중 **테스트 실패 오류(AssertionError, NullPointerException 등)로 실패했을 때, 그 원인을 심층 분석하고 해결하기 위해 참조**하는 문제 해결 가이드입니다.
>
