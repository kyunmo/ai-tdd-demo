...
```bash
./gradlew test --tests "com.nhcard.al.tt.service.UserServiceTest.should_createUser_when_validRequest"
```
...
| 기준 | 합격 | 불합격 |
|---|---|---|
| 실행 결과 | `BUILD SUCCESSFUL` | `BUILD FAILED` |
| 실패 테스트 수 | 0개 | 1개 이상 |
| 스킵 테스트 수 | 0개 | 1개 이상 (`@Disabled` 금지) |
| 실행 시간 | 단위테스트: 클래스당 5초 이내 | 비정상 지연 |
...
```java
// 1. 클래스 어노테이션 확인
@ExtendWith(MockitoExtension.class)  // 반드시 포함
...
userService.getUserById(1L);  // 위 두 Mock을 호출하지 않는 메서드

// 좋은 예시 - 테스트 대상 메서드가 실제 호출하는 Mock만 설정
when(userMapper.findById(1L)).thenReturn(user);  // getUserById가 호출하는 메서드
...
java.lang.NullPointerException
    at com.nhcard.al.tt.service.UserService.getUserById(UserService.java:25)
```
...
```java
// 나쁜 예시 - Mock 반환값 미설정
// when(userMapper.findById(1L)).thenReturn(user);  // 누락!
User result = userService.getUserById(1L);  // NPE 발생

// 좋은 예시 - 소스 코드 흐름을 따라 필요한 Mock 모두 설정
when(userMapper.findById(1L)).thenReturn(user);  // 소스 코드에서 호출하는 메서드
...
when(userMapper.findById(1L)).thenReturn(
    new User(1L, "테스트사용자", "test@example.com")
);
