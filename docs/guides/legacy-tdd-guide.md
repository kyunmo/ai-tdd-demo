...
## 목차
1.  [테스트 코드의 필요성](#1-테스트-코드의-필요성)
    1.  [레거시 프로젝트의 현실](#11-레거시-프로젝트의-현실)
    2.  [테스트 자동화의 이점](#12-테스트-자동화의-이점)
    3.  [AI 에이전트 활용의 필요성](#13-ai-에이전트-활용의-필요성)
2.  [TDD 도입 목표 및 범위](#2-tdd-도입-목표-및-범위)
    1.  [Phase 1: 기존 소스에 테스트 코드 생성 (Test-After)](#phase-1-기존-소스에-테스트-코드-생성-test-after)
    2.  [Phase 2: 신규 기능을 TDD로 개발 (Test-First)](#phase-2-신규-기능을-tdd로-개발-test-first)
    3.  [최종 목표](#최종-목표)
3.  [AI TDD 환경 설정 가이드](#3-ai-tdd-환경-설정-가이드)
    1.  [프로젝트에 에이전트 파일 배치](#31-프로젝트에-에이전트-파일-배치)
    2.  [에이전트 역할 및 동작 방식](#32-에이전트-역할-및-동작-방식)
    3.  [스킬 문서 배치 (docs/ai-tdd-skills/)](#33-스킬-문서-배치-docsai-tdd-skills)
    4.  [프로젝트 설정 파일 작성 (.claude.md)](#34-프로젝트-설정-파일-작성-claude-md)
    5.  [빌드 설정 (JaCoCo, PIT)](#35-빌드-설정-jacoco-pit)
    6.  [에이전트 동작 확인](#36-에이전트-동작-확인)
    7.  [에이전트 사용법](#37-에이전트-사용법)
4.  [Phase 1: 기존 소스 테스트 생성 (Test-After)](#4-phase-1-기존-소스-테스트-생성-test-after)
    1.  [대상 클래스 선정](#41-대상-클래스-선정)
    2.  [단일 클래스 테스트 생성 실습](#42-단일-클래스-테스트-생성-실습)
    3.  [생성된 테스트 코드 평가 및 학습](#43-생성된-테스트-코드-평가-및-학습)
    4.  [(선택) 리뷰 에이전트로 품질 확인](#44-선택-리뷰-에이전트로-품질-확인)
5.  [Phase 2: 신규 기능 TDD 개발 (Test-First)](#5-phase-2-신규-기능-tdd-개발-test-first)
    1.  [Red-Green-Refactor 사이클 (실패-통과-개선)](#51-red-green-refactor-사이클-실패-통과-개선)
    2.  [시나리오: 기존 프로젝트에 "즐겨찾기" 기능 TDD로 추가](#52-시나리오-기존-프로젝트에-즐겨찾기-기능-tdd로-추가)
6.  [트러블슈팅 및 FAQ](#6-트러블슈팅-및-faq)
    1.  [Q1. 에이전트가 소스 파일을 못 찾아요](#q1-에이전트가-소스-파일을-못-찾아요)
    2.  [Q2. 생성된 테스트가 컴파일 실패해요](#q2-생성된-테스트가-컴파일-실패해요)
    3.  [Q3. 커버리지가 목표에 미달해요](#q3-커버리지가-목표에-미달해요)
    4.  [Q4. Mock 설정이 실제 코드와 안 맞아요](#q4-mock-설정이-실제-코드와-안-맞아요)
    5.  [Q5. 에이전트가 동작하지 않아요](#q5-에이전트가-동작하지-않아요)
    6.  [Q6. 폐쇄망에서 Gradle 의존성을 받지 못해요](#q6-폐쇄망에서-gradle-의존성을-받지-못해요)
7.  [TDD 문화 확산 전략](#7-tdd-문화-확산-전략)
    1.  [팀 도입 단계별 로드맵](#71-팀-도입-단계별-로드맵)
    2.  [코드 리뷰에 테스트 포함 기준](#72-코드-리뷰에-테스트-포함-기준)
    3.  [점진적 도입 전략](#73-점진적-도입-전략)
...
#### Step 1: Red — 테스트 먼저 작성 (AI로 강화된 방식)

테스트 주도 개발(TDD)의 첫 단계는 **실패하는 테스트를 작성**하는 것입니다.

기존 방식에서는 테스트 대상 클래스나 메서드가 없어 컴파일조차 실패하는 상태에서 시작했습니다. 하지만 **AI TDD 에이전트를 활용하면 이 과정이 더욱 효율적으로 개선**됩니다.

AI에게 테스트 생성을 요청하면, 단순히 테스트 코드만 만드는 것을 넘어 **테스트 대상이 되는 서비스 클래스의 기본 구조(skeleton)까지 함께 생성**해줍니다.

> **심화: AI의 코드 생성 범위는 상황에 따라 달라집니다**
>
> AI가 생성하는 코드의 상세 수준은 프로젝트의 기존 코드, 즉 **참고할 만한 소스가 있는지 여부**에 따라 달라집니다. 개발자는 두 가지 주요 시나리오를 이해해야 합니다.
>
...
> AI가 생성한 구현 코드는 어떤 경우든 **검증되지 않은 '초안'**일 뿐이며, **반드시 개발자가 직접 전체 코드를 검토하고 수정해야 합니다.**
>
> 비즈니스 규칙의 정확성, 예외 처리의 적절성, 보안 측면 등을 꼼꼼히 확인하는 과정이 필수적입니다. 이 점을 인지하지 못하면 예상치 못한 버그가 발생할 수 있으니 각별히 주의해야 합니다.

이렇게 AI가 생성한 코드는 **컴파일은 통과하지만, 핵심 비즈니스 로직의 정확성이 보장되지 않았으므로 어설션(assertion) 단계에서 실패하거나, 혹은 잘못된 로직으로 인해 우연히 통과할 수 있습니다.** 기능적으로 완전하지 않거나 검증되지 않은 이 상태가 바로 우리가 목표하는 **'Red'** 상태입니다.

이 접근법은 컴파일 오류 해결에 드는 시간을 절약하고, 개발자가 즉시 핵심 로직 검토 및 구현에 집중할 수 있도록 돕습니다.
...
1.  **테스트 클래스 (`FavoriteServiceTest.java`)**: 요구사항에 기반한 테스트 메서드들이 포함됩니다.
2.  **서비스 클래스 (`FavoriteService.java`)**: 기본 구조를 넘어, 때로는 구현 로직까지 포함될 수 있습니다.
3.  **엔티티 클래스 (`Favorite.java`)**: (필요시) AI가 추론하여 생성할 수 있습니다.

이제 생성된 테스트 코드를 검증 스크립트로 실행하여 의도대로 실패하는 **Red** 상태를 확인합니다.
